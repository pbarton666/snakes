...
raw_image = Image.open(idir+iname)
...
#data[..., :-1][replace.T] = (0,0,0)  #this would leave the alpha channel intact
...
recovered_image = Image.open(idir+saveas)
...
Returns a histogram for those parts of the image where the mask image is non-zero. 
The mask image must have the same size as the image, and be either a bi-level 
image (mode “1”) or a greyscale image (“L”).'''

#There's a post that might work:  result = image.convert('P', palette=Image.ADAPTIVE, colors=5)
...
#reduc2 = reduc1.convert() #turns it to rgb
